...
describe('fetch-github-repo', () => {
  ...
  it('downloads the main branch by default', async () => {
    await Download('NimbleWing/fetch-github-repo', 'test/tmp');
  ...
  it('download branches too', async () => {
    await Download('NimbleWing/fetch-github-repo#fixtures', 'test/tmp');
    const actual = Read('test/tmp');
  ...
  it('not found by error repo', async () => {
    try {
      await Download('NimbleWin/fetch-github-repo#fixtures', 'test/tmp');
    } catch ($err) {
